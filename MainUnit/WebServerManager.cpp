...
.progress-bar{height:100%;background:linear-gradient(90deg,#4CAF50,#45a049);border-radius:12px;transition:width 0.5s ease;position:relative}
...
setInterval(updateStatus,10000);setInterval(updateDispensingStatus,1000);updateStatus();updateDispensingStatus();
</script></body></html>
)rawliteral";
...
WebServerManager::WebServerManager(PIRSensor* pir, RelayController* relay, Flashlight* flashlight, DispensingController* dispensing)
    : server(80), pirSensor(pir), relayController(relay), flashlightController(flashlight), dispensingController(dispensing), wifiMultiPtr(nullptr), flowRate(0), totalVolume(0), flowError(false), flowDataCallback(nullptr) {
}
    ...
    server.on("/flow/update", HTTP_ANY, [this](AsyncWebServerRequest *request) {
        if (request->method() == HTTP_GET) {
            if (request->hasParam("flowRate") && request->hasParam("totalVolume") && request->hasParam("error")) {
                ...
                updateFlowData(newFlowRate, newTotalVolume, newError);
