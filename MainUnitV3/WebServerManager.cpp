...
WebServerManager::WebServerManager(PIRSensor* pir, RelayController* relay, Flashlight* flashlight, DispensingController* dispensing)
    : server(80), pirSensor(pir), relayController(relay), flashlightController(flashlight), dispensingController(dispensing), wifiMultiPtr(nullptr), flowRate(0), totalVolume(0), flowError(false), pulseCount(0), flowDataCallback(nullptr) {
}
...
void WebServerManager::setupRoutes() {
    ...
    // Flow sensor dispensing status endpoint - tells flow sensor if dispensing is active
        ...
        if (dispensingController) {
            for (int pumpId = 1; pumpId <= 2; pumpId++) {
                if (dispensingController->isDispensing(pumpId)) {
                    anyDispensing = true;
                    activePump = pumpId;
                    targetVolume = dispensingController->getTargetVolume(pumpId);
                    break; // Use first active pump
                }
            }
        }
        
        doc["isDispensing"] = anyDispensing;
